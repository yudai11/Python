...
def adversarial_validation(train_X, test_X):
    ...
    # Get the threshold for the top 20% most test-like instances
    threshold = np.percentile(prob, 80)

    # Return the indices and probabilities of top 20% most test-like samples in train_X
    top_20_mask = prob >= threshold
    top_20_indices = np.where(top_20_mask)[0]
    # top_20_probs = prob[top_20_mask]
...
def main():
    ...
    vals_indices = adversarial_validation(train_x, test_x)
    vals_x = train_x[vals_indices]
    vals_y = train_y[vals_indices]

    # 1. データの結合
    X_combined = np.concatenate((train_x, vals_x), axis=0)
    y_combined = np.concatenate((train_y, vals_y), axis=0)

    # 2. test_fold の作成
    test_fold = np.concatenate((
        np.full(len(train_x), -1),  # 訓練データのインデックス
        np.zeros(len(vals_x))      # 検証データのインデックス
    ))

    # 3. PredefinedSplit の作成
    ps = PredefinedSplit(test_fold)

    # テストに近いサンプルを重点的に学習
    # sample_weights = domain_scores + 0.05
