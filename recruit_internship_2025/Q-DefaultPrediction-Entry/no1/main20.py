...
def main():
    ...
    # 前処理
    train_x = preprocess(train_df, fit_scaler=True)
    test_x = preprocess(test_df, fit_scaler=True)

    # Adversarial Validation
    vals_indices, w_trains = adversarial_validation(train_x, test_x)
    w_trains = (w_trains + 0.001) * 100.0
    ...
    model.fit(X=train_x, y=train_y, sample_weight=w_trains)
